...
const AboutPHView: React.FC<{ lang: Language; onAction: () => void }> = ({ lang, onAction }) => {
  ...
  const transports = [
    ...
    },
    {
      ...
      img: 'https://images.unsplash.com/photo-1516664468114-1f7c00185121?q=80&w=800'
    },
    {
      id: 'rentals',
        ...
        <div className="bg-gradient-to-br from-blue-900 to-blue-950 rounded-[4rem] p-12 md:p-24 text-center text-white relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 space-y-8">
            ...
            <p className="text-white/70 max-w-2xl mx-auto font-bold text-lg md:text-xl">
              {lang === 'AR' ? 'دعنا نصمم لك البرنامج السياحي الذي طالما حلمت به في جنة الفلبين.' : 'Let us design the itinerary you have always dreamed of in the paradise of Philippines.'}
            </p>
            <button 
              onClick={onAction}
...
const AdminDashboardView: React.FC<{ 
  ...
  const [activeTab, setActiveTab] = useState<'GENERAL' | 'ISLANDS' | 'MANILA' | 'SHOPPING' | 'RESTAURANTS' | 'ACTIVITIES' | 'TRANSLATIONS'>('GENERAL');
  ...
  const tabs = [
    ...
    { id: 'TRANSLATIONS', label: lang === 'AR' ? 'القوائم والنصوص' : 'Menus & Texts' },
  ];
    ...
    <div className={`py-12 container mx-auto px-4 ${lang === 'AR' ? 'text-right' : 'text-left'}`}>
      <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={onLogout} className="bg-blue-900 text-white px-6 py-3 rounded-2xl font-black shadow-xl hover:bg-red-600 transition-colors">🚪 {lang === 'AR' ? 'خروج' : 'Logout'}</button>
        </div>
      </div>
        ...
        {activeTab === 'TRANSLATIONS' ? (
          <div className="space-y-10 animate-in fade-in duration-500">
             <div className="flex justify-between items-center mb-8">
               <h3 className="text-2xl font-black text-blue-900">{lang === 'AR' ? 'تعديل نصوص القوائم والموقع' : 'Edit Menu Texts & Labels'}</h3>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
               {Object.keys(siteData.translations).map((key) => (
                 <div key={key} className="bg-gray-50 p-6 rounded-[2rem] space-y-4 border border-gray-100">
                    <span className="text-[10px] font-black text-blue-900 uppercase tracking-widest">{key}</span>
                    <div className="space-y-4">
                      <div className="flex items-center gap-4">
                        <span className="text-[10px] font-bold text-gray-400 w-8">AR</span>
                        <input 
                          value={siteData.translations[key].AR} 
                          onChange={(e) => onUpdate({...siteData, translations: {...siteData.translations, [key]: {...siteData.translations[key], AR: e.target.value}}})} 
                          className="flex-grow p-3 bg-white rounded-xl border border-transparent focus:border-blue-200 outline-none" 
                        />
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="text-[10px] font-bold text-gray-400 w-8">EN</span>
                        <input 
                          value={siteData.translations[key].EN} 
                          onChange={(e) => onUpdate({...siteData, translations: {...siteData.translations, [key]: {...siteData.translations[key], EN: e.target.value}}})} 
                          className="flex-grow p-3 bg-white rounded-xl border border-transparent focus:border-blue-200 outline-none" 
                        />
                      </div>
                    </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="col-span-1 md:col-span-2 space-y-6">
                 <div className="flex justify-between items-center bg-blue-50/50 p-6 rounded-[2rem] border border-blue-100">
                   ...
                   <button onClick={triggerFileUpload} className="bg-red-600 text-white px-8 py-4 rounded-2xl font-black text-xs shadow-xl active:scale-95 transition-all flex items-center gap-2">📸 {lang === 'AR' ? 'إضافة / رفع صورة جديدة' : 'Add / Upload New Image'}</button>
                   <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept="image/*" multiple={activeTab !== 'GENERAL'} />
                 </div>
            ...
            <div className="flex gap-4 pt-10 border-t border-gray-100">
              <button onClick={() => saveEdit(activeTab === 'GENERAL' ? 'heroSlides' : activeTab.toLowerCase() as any)} className="bg-green-600 text-white px-16 py-5 rounded-2xl font-black shadow-xl hover:scale-105 transition-all">✅ {lang === 'AR' ? 'حفظ التغييرات النهائية' : 'Save All Changes'}</button>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
             <div className="flex flex-col md:flex-row justify-between items-center gap-4">
               ...
               <button onClick={() => setEditingItem({ name: {AR:'', EN:'', PH:''}, description: {AR:'', EN:'', PH:''}, images: [], location: {AR:'', EN:'', PH:''}, bestFor: {AR:'', EN:'', PH:''}, category: activeTab === 'GENERAL' ? 'ISLAND' : (activeTab as any) })} className="bg-blue-900 text-white px-8 py-4 rounded-2xl font-black text-sm shadow-xl hover:bg-red-600 transition-colors">+ {lang === 'AR' ? 'إضافة عنصر جديد' : 'Add New'}</button>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               {(activeTab === 'GENERAL' ? siteData.heroSlides : (siteData as any)[activeTab.toLowerCase()]).map((item: any) => (
                 <div key={item.id} onClick={() => setEditingItem(item)} className={`p-6 rounded-[2.5rem] border-2 cursor-pointer group transition-all duration-300 ${item.hidden ? 'bg-gray-50 border-dashed border-gray-200 opacity-60' : 'bg-white border-transparent shadow-lg hover:shadow-2xl hover:border-blue-100'} flex flex-col justify-between`}>
                   ...
                   <div className="flex gap-2">
                     ...
                     <button onClick={(e) => { e.stopPropagation(); toggleVisibility(activeTab === 'GENERAL' ? 'heroSlides' : activeTab.toLowerCase() as any, item.id); }} className="p-3 bg-gray-100 rounded-xl hover:bg-gray-200 transition">
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden group hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col h-full">
      ...
      <div className={`p-8 space-y-4 flex-grow flex flex-col ${lang === 'AR' ? 'text-right' : 'text-left'}`}>
        ...
        <button onClick={onBook} className="w-full bg-blue-900 text-white py-4 rounded-2xl font-black text-sm hover:bg-red-600 transition shadow-xl active:scale-95">{getButtonText()}</button>
      </div>
    </div>
  );
};
...
const AdminLoginView: React.FC<{ 
  onLogin: (s: boolean) => void; 
  onVisitorEntry: () => void; 
  lang: Language;
  setLang: (l: Language) => void;
}> = ({ onLogin, onVisitorEntry, lang, setLang }) => {
  ...
  const strings = {
    portalTitle: { AR: 'بوابة مابوهاي السياحية', EN: 'Mabuhay Tourism Portal' },
    portalSub: { AR: 'مرحباً بك في المنصة الشاملة لسياحة الفلبين. يرجى اختيار طريقة الدخول للمتابعة.', EN: 'Welcome to the PH tourism platform. Please select your entry method.' },
    adminTab: { AR: 'تسجيل دخول الإدارة', EN: 'Admin Management Login' },
    adminSub: { AR: 'صلاحيات كاملة للتحكم في المحتوى', EN: 'Full content management access' },
    email: { AR: 'البريد الإلكتروني', EN: 'Email Address' },
    password: { AR: 'كلمة المرور', EN: 'Password' },
    loginBtn: { AR: 'دخول الأدمن', EN: 'Admin Login' },
    visitorTab: { AR: 'دخول الزوار والمسافرين', EN: 'Visitor & Traveler Entry' },
    visitorSub: { AR: 'تصفح الوجهات والفعاليات والحجز', EN: 'Browse destinations and book' },
    visitorQuote: { AR: '"اكتشف جمال 7,641 جزيرة برؤية عربية أصيلة"', EN: '"Discover the beauty of 7,641 islands"' },
    enterAsVisitor: { AR: 'الدخول كزائر', EN: 'Enter as Visitor' },
    error: { AR: 'خطأ في البيانات!', EN: 'Invalid Credentials!' }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-950 to-blue-900 flex items-center justify-center p-4 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-96 h-96 bg-red-600/10 rounded-full -mr-20 -mt-20 blur-[100px]"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-blue-400/10 rounded-full -ml-20 -mb-20 blur-[100px]"></div>

      <div className="bg-white p-6 md:p-16 rounded-[4rem] shadow-2xl w-full max-w-4xl border border-white/20 relative z-10 flex flex-col items-center">
        
        {/* Language Switcher */}
        <div className="absolute top-8 right-8 md:top-12 md:right-12 flex gap-2">
            <button 
                onClick={() => setLang('AR')} 
                className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${lang === 'AR' ? 'bg-blue-900 text-white shadow-lg shadow-blue-900/20' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
            >عربي</button>
            <button 
                onClick={() => setLang('EN')} 
                className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${lang === 'EN' ? 'bg-blue-900 text-white shadow-lg shadow-blue-900/20' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
            >English</button>
        </div>

        <div className="mb-10 scale-110 md:scale-125"><PHLogo /></div>
        
        <h2 className="text-2xl md:text-5xl font-black text-blue-900 text-center mb-2">
          {strings.portalTitle[lang]}
        </h2>
        <p className="text-gray-400 font-bold text-center mb-12 max-w-md text-sm md:text-base">
          {strings.portalSub[lang]}
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 w-full">
          <div className="space-y-8 bg-blue-50/30 p-6 md:p-8 rounded-[3rem] border border-blue-100 flex flex-col">
            <div className="text-center">
               <h3 className="text-lg md:text-xl font-black text-blue-900 mb-1">{strings.adminTab[lang]}</h3>
               <p className="text-[10px] text-gray-500 font-bold">{strings.adminSub[lang]}</p>
            </div>
            <form onSubmit={handleAdminSubmit} className="space-y-4">
              <input 
                ...
                className={`w-full p-4 md:p-5 bg-white rounded-2xl outline-none border-2 transition-all ${error ? 'border-red-500 animate-shake' : 'border-transparent focus:border-blue-300'}`} 
                required 
              />
              <div className="relative">
                <input 
                  ...
                  className={`w-full p-4 md:p-5 bg-white rounded-2xl outline-none border-2 transition-all ${error ? 'border-red-500' : 'border-transparent focus:border-blue-300'}`} 
                  required 
                />
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-blue-900 transition">
                  {showPass ? <ICONS.EyeOff /> : <ICONS.Eye />}
                </button>
              </div>
              {error && <p className="text-red-500 text-[10px] font-black text-center">{strings.error[lang]}</p>}
              <button 
                disabled={isLoggingIn}
                className="w-full bg-blue-900 text-white py-4 md:py-5 rounded-3xl font-black text-lg shadow-xl hover:bg-blue-800 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                {isLoggingIn ? '...' : strings.loginBtn[lang]}
              </button>
            </form>
          </div>

          <div className="space-y-8 bg-red-50/30 p-6 md:p-8 rounded-[3rem] border border-red-100 flex flex-col justify-between">
            <div className="text-center">
               <h3 className="text-lg md:text-xl font-black text-red-600 mb-1">{strings.visitorTab[lang]}</h3>
               <p className="text-[10px] text-gray-500 font-bold">{strings.visitorSub[lang]}</p>
            </div>
            <div className="p-4 bg-white/50 rounded-2xl text-[10px] md:text-xs font-medium text-gray-500 text-center italic">
                {strings.visitorQuote[lang]}
            </div>
            <button 
              onClick={onVisitorEntry}
              className="w-full bg-red-600 text-white py-6 md:py-8 rounded-3xl font-black text-xl md:text-2xl shadow-xl hover:bg-red-700 active:scale-95 transition-all flex flex-col items-center gap-2 md:gap-3"
            >
              <span className="text-2xl md:text-3xl">🌴</span>
              <span>{strings.enterAsVisitor[lang]}</span>
            </button>
          </div>
        </div>

        <div className="mt-12 pt-8 border-t border-gray-100 w-full text-center">
          <p className="text-[10px] text-gray-300 font-bold uppercase tracking-widest">© Mabuhay Tourism Philippines 2026</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className={`min-h-screen bg-[#FDFDFF] pb-24 md:pb-32 ${lang === 'AR' ? "font-['Cairo']" : "font-sans"}`}>
        ...
        <nav className="sticky top-0 z-[100] bg-white/80 backdrop-blur-2xl border-b border-gray-100 shadow-sm">
          <div className="container mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <div className="hidden lg:flex items-center gap-8">
              {isAdminLoggedIn && (
                <button onClick={() => navigateTo('ADMIN_DASHBOARD')} className="text-[10px] font-black tracking-widest text-blue-900 bg-blue-50 px-5 py-2 rounded-2xl shadow-xl border border-blue-100 hover:bg-blue-900 hover:text-white transition-all animate-pulse">ADMIN DASHBOARD</button>
              )}
              ...
              <button onClick={() => { setIsAdminLoggedIn(false); navigateTo('ADMIN_LOGIN'); }} className="text-[10px] font-black text-gray-300 hover:text-red-600 transition-colors uppercase">Logout/Switch</button>
            </div>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[200] bg-white p-10 lg:hidden animate-in fade-in slide-in-from-right duration-300">
           ...
           <div className="flex flex-col gap-8">
              ...
              {isAdminLoggedIn && (
                <button onClick={() => navigateTo('ADMIN_DASHBOARD')} className="text-xl font-black text-blue-900 text-right border-y py-4">DASHBOARD</button>
              )}
              <button onClick={() => navigateTo('ADMIN_LOGIN')} className="text-lg font-bold text-gray-400 text-right uppercase mt-12">LOGOUT / SWITCH USER</button>
           </div>
        </div>
      )}
      ...
      <main>
        ...
        {view === 'ADMIN_LOGIN' && (
          <AdminLoginView 
            onLogin={(success) => { if (success) { setIsAdminLoggedIn(true); navigateTo('HOME'); } }} 
            onVisitorEntry={() => { setIsAdminLoggedIn(false); navigateTo('HOME'); }} 
            lang={lang} 
            setLang={setLang}
          />
        )}
        
        {view === 'ADMIN_DASHBOARD' && isAdminLoggedIn && (
          <AdminDashboardView 
            siteData={siteData} 
            onUpdate={updateSiteData} 
            onUndo={undoDataChange} 
            onReset={resetToDefault} 
            onLogout={() => { setIsAdminLoggedIn(false); navigateTo('ADMIN_LOGIN'); }}
            lang={lang} 
          />
        )}
      </main>
      ...
      {view !== 'ADMIN_LOGIN' && (
        <>
          <PlanningBar lang={lang} onAction={() => navigateTo('BOOKING')} isVisible={!isAdminLoggedIn || (view !== 'ADMIN_DASHBOARD' && view !== 'ADMIN_LOGIN')} />
          <Footer lang={lang} navigateTo={navigateTo} t={siteData.translations} isAdminLoggedIn={isAdminLoggedIn} />
          <AIChatbot lang={lang} />
        </>
      )}
    </div>
...
const ListView: React.FC<{ title: string; subtitle: string; items: Attraction[]; banner: string; onBook: () => void; lang: Language; showFilters?: boolean }> = ({ title, subtitle, items, banner, onBook, lang, showFilters }) => {
  ...
  const filteredItems = useMemo(() => items.filter(item => (filter.city === 'ALL' || item.location[lang].includes(filter.city)) && (filter.level === 'ALL' || (item.level && item.level[lang] === filter.level))), [items, filter, lang]);
  const cities = useMemo(() => ['ALL', ...Array.from(new Set(items.map(i => i.location[lang].split(' ')[0])))], [items, lang]);
      ...
      {showFilters && (
        <div className="container mx-auto px-4 mb-12">
          <div className="bg-white/50 backdrop-blur-md p-6 rounded-[2.5rem] border border-blue-50 shadow-sm flex flex-wrap gap-4 items-center justify-center">
            <select className="bg-white px-6 py-3 rounded-2xl border border-gray-100 outline-none text-xs font-bold shadow-sm" value={filter.city} onChange={e => setFilter({...filter, city: e.target.value})}>
              <option value="ALL">{lang === 'AR' ? 'جميع المدن' : 'All Cities'}</option>
              {cities.filter(c => c !== 'ALL').map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select className="bg-white px-6 py-3 rounded-2xl border border-gray-100 outline-none text-xs font-bold shadow-sm" value={filter.level} onChange={e => setFilter({...filter, level: e.target.value})}>
...
const BookingView: React.FC<{ navigateTo: (v: any) => void; lang: Language }> = ({ lang }) => {
  const [done, setDone] = useState(false);
  if (done) return <div className="h-[40vh] flex flex-col items-center justify-center font-black text-3xl text-blue-900 animate-bounce">{lang === 'AR' ? 'تم استلام طلبك بنجاح!' : 'Request Received Successfully!'}</div>;
  return (
    <div className="py-12 container mx-auto px-4 max-w-xl text-center">
      <form onSubmit={(e) => { e.preventDefault(); setDone(true); }} className="space-y-6">
        <input placeholder={lang === 'AR' ? 'الاسم بالكامل' : 'Full Name'} className="w-full p-5 bg-white border rounded-2xl shadow-sm outline-none" required />
        <button type="submit" className="w-full bg-red-600 text-white py-5 rounded-3xl font-black text-xl shadow-2xl hover:scale-105 active:scale-95 transition-all">{lang === 'AR' ? 'إرسال طلب الحجز' : 'Send Booking Request'}</button>
      </form>
    </div>
  );
};
...
const AIChatbot: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const [msg, setMsg] = useState([{ role: 'bot', text: 'Mabuhay!' }]);
  ...
  return (
    <div className={`fixed bottom-24 md:bottom-32 ${lang === 'AR' ? 'left-8' : 'right-8'} z-[150]`}>
      {open && (
        <div className="absolute bottom-20 inset-x-0 md:w-96 h-[32rem] bg-white rounded-[2.5rem] shadow-2xl border flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          <div className="bg-blue-900 p-6 text-white font-black flex justify-between"><span>Mabuhay AI</span><button onClick={() => setOpen(false)}>&times;</button></div>
          <div className="flex-grow p-6 overflow-y-auto space-y-4">
            {msg.map((m, i) => (<div key={i} className={`p-4 rounded-xl text-xs ${m.role === 'user' ? 'bg-red-600 text-white ml-8' : 'bg-gray-100 text-blue-900 mr-8'}`}>{m.text}</div>))}
          </div>
          <div className="p-4 border-t flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handle()} className="flex-grow p-3 bg-gray-100 rounded-xl outline-none" />
            <button onClick={handle} className="bg-blue-900 text-white px-4 rounded-xl">✓</button>
          </div>
        </div>
      )}
      <button onClick={() => setOpen(!open)} className="w-16 h-16 bg-blue-900 rounded-3xl text-white shadow-2xl flex items-center justify-center font-bold">AI</button>
    </div>
  );
};

const Footer: React.FC<{ lang: Language; navigateTo: (v: any) => void; t: any; isAdminLoggedIn: boolean }> = ({ lang, navigateTo, t }) => (
  <footer className="bg-white border-t border-gray-100 pt-24 pb-12">
    <div className="container mx-auto px-4">
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 ${lang === 'AR' ? 'text-right' : 'text-left'} mb-20`}>
        <div className="space-y-6">
          ...
          <p className="text-gray-500 text-sm leading-relaxed font-bold">
            {lang === 'AR' ? 'موبهاي للسياحة - بوابتك لاستكشف أجمل جزر الفلبين وتجربة حياة المدن العصرية بلمسة عربية أصيلة.' : 'Mabuhay Travel - Your gateway to exploring the most beautiful islands in the PH.'}
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col">
              <span className="text-[10px] text-gray-400 uppercase font-black">{lang === 'AR' ? 'البريد الإلكتروني' : 'Email'}</span>
      ...
      <div className={`pt-12 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8 ${lang === 'AR' ? 'text-right' : 'text-left'}`}>
        <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">© Mabuhay Tourism 2026</p>
      </div>
    </div>
  </footer>
);
